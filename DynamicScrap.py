...
class DynamicScrapeAgent:
    ...
    Raises:
        ...
        ValueError: If the client is not an OpenAI instance.
    ...
    def __init__(
        ...
        if not isinstance(client, OpenAI):
            raise ValueError("client must be an OpenAI instance.")
    ...
    def ask_ai(self, prompt: str, temperature: float = 0) -> str:
        """
    ...
    def safe_json_loads(self, text: str, fallback: Any) -> Any:
        """
    ...
    def run_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            def try_multi_locator_hover_or_click(kind: str):
                attempts = []
                if target:
                ...
                for idx, (_, loc) in enumerate(attempts):
                    ...
                    try:
                        if kind == "hover":
                            loc.hover(force=True, timeout=current_timeout)
                        elif kind == "click":
                            loc.click(timeout=current_timeout)
                        elif kind == "double_click":
                            loc.dblclick(timeout=current_timeout)
                        elif kind == "right_click":
                            loc.click(button="right", timeout=current_timeout)
                        return
            ...
            if action == "hover":
                try_multi_locator_hover_or_click("hover")
            elif action == "click":
                try_multi_locator_hover_or_click("click")
            elif action == "double_click":
                try_multi_locator_hover_or_click("double_click")
            elif action == "right_click":
                try_multi_locator_hover_or_click("right_click")
            elif action == "type":
                ...
                if wait_mode in {"load", "domcontentloaded", "networkidle"}:
                    self.page.wait_for_load_state(wait_mode, timeout=timeout)
                else:
                    locator = locator_from_strategy()
                    locator.wait_for(state="visible", timeout=timeout)
            elif action == "scroll":
                self.page.mouse.wheel(0, int(value or 1400))
                self.page.wait_for_timeout(1000)
            elif action == "navigate":
                self.page.goto(value or target, wait_until="domcontentloaded")
            elif action == "back":
                ...
                payload = self.safe_json_loads(value, fallback={})
                ...
                src.drag_to(dst, timeout=timeout)
            else:
                ...
                if aliased:
                    ...
                    return self.run_step(canonical_step)
    ...
    def _collect_visible_inputs(self) -> List[Dict[str, str]]:
        """
...
() => {
    ...
    def _llm_pick_search_box(self, inputs: List[Dict[str, str]]) -> Optional[str]:
        """
...
"""
        ...
        parsed = self.safe_json_loads(raw, fallback={})
    ...
    def detect_search_and_run(self, query: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            box.fill("", timeout=3000)   # clear any pre-filled text
            box.press_sequentially(query, delay=45)    # human-like typing
    ...
    def collect_visible_links(self) -> List[Dict[str, str]]:
        """
...
() => {
    ...
    def required_action_types(self, action_instruction: str) -> List[str]:
        """
...
No markdown. JSON array only."""
        ...
        if isinstance(parsed, list):
            return [x.lower().strip() for x in parsed if isinstance(x, str)]
        return []
    ...
    def check_task_possibility(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """
...
- No markdown. No text outside JSON."""
        ...
        if isinstance(parsed, dict):
            steps = self.normalize_steps(parsed.get("steps", []))
            return {"steps": steps, "raw": raw}
        # backward compatibility if model still returns array
        steps = self.normalize_steps(parsed)
        return {"steps": steps, "raw": raw}

    def fallback_steps_from_instruction(self, instruction: str) -> List[Dict[str, Any]]:
        """
...
- If instruction is unclear, return []."""
        raw = self.ask_ai(prompt)
        parsed = self.safe_json_loads(raw, fallback=[])
        return self.normalize_steps(parsed) if isinstance(parsed, list) else []

    def extract_hover_target_from_instruction(self, instruction: str) -> str:
        """
...
- Return [] if nothing matches."""
        raw = self.ask_ai(prompt)
        data = self.safe_json_loads(raw, fallback=None)
...
- No prose, no markdown, JSON only."""
    ...
    def validate_extraction_spec(self, spec: Dict[str, Any]) -> bool:
        """
...
- JSON only, no markdown."""
            raw = self.ask_ai(classify_prompt)
            parsed = self.safe_json_loads(raw, fallback={})
            if isinstance(parsed, dict):
        ...
        def _identify_item_links() -> List[Dict[str, str]]:
...
No markdown. No explanation."""
            raw = self.ask_ai(prompt)
            indices = self.safe_json_loads(raw, fallback=[])
        ...
        spec = self.generate_extraction_spec(task, links)
        self.last_extraction_spec = spec
        if self.validate_extraction_spec(spec):
            self.logs.append("dynamic_spec: valid")
            dynamic_rows = self.collect_by_dynamic_spec(spec, task)
            if dynamic_rows:
                ...
                if title_fields or url_fields:
                    ...
                    if needs_enrich:
                        item_links = _identify_item_links()
                        self.logs.append(
        ...
        scope = self.choose_source_scope(task, links)
...
- No explanation outside JSON."""
        ...
        parsed = self.safe_json_loads(raw, fallback={})
        if isinstance(parsed, dict):
            return self.normalize_steps(parsed.get("steps", []))
        return self.normalize_steps(parsed)

    def observe_failure_context(self) -> Dict[str, Any]:
        """
    ...
    def run_task(self) -> Dict[str, Any]:
        ...
        def check_timeout():
            if time.time() - start_time > total_timeout_sec:
                raise TimeoutError(f"Task timed out after {total_timeout_sec} seconds.")

        self.start()
        check_timeout()
        self.open(self.task["url"])

        check_timeout()
        feasibility = self.check_task_possibility(self.task)
        self.logs.append(f"feasibility: {feasibility}")
        if not feasibility["possible"]:
            observe = self.observe_failure_context()
            return {
                "ok": False,
                "data": [],
                "diagnostics": {
        ...
        if search_query:
            check_timeout()
            search_result = self.detect_search_and_run(search_query)
...
Return {{"skip": false}} if it requires any additional browser actions beyond searching."""
            check_timeout()
            _skip_raw = self.ask_ai(_skip_prompt)
            _skip_parsed = self.safe_json_loads(_skip_raw, fallback={})
        ...
        if _search_only_instruction:
            ...
            check_timeout()
            required_actions = self.required_action_types(
                action_instruction
            )
            check_timeout()
            plan_result = self.plan_actions(self.task)
            planned_steps = plan_result["steps"]
            plan_validation = self.validate_planned_steps(
                planned_steps, 
                required_actions
            )
            ...
            if not plan_validation["ok"]:
                ...
                check_timeout()
                replan_result = self.plan_actions(
                    self.task, 
            ...
            if not plan_validation["ok"] and action_instruction:
                check_timeout()
                planned_steps = self.fallback_steps_from_instruction(
                    action_instruction
                )
                if planned_steps:
                    self.logs.append(
        ...
        for step in planned_steps:
            ...
            if (not step_result.get("ok")) and step.get("action") == "hover":
                ...
                check_timeout()
                fallback_target = self.extract_hover_target_from_instruction(
                    action_instruction
                )
                if fallback_target and fallback_target != (step.get("target") or ""):
                    ...
                    check_timeout()
                    retry_result = self.run_step(retry_step)
                    self.logs.append(
        ...
        if hover_will_be_performed:
            check_timeout()
            post_hover_links = self.collect_visible_links()
            pre_hover_keys = {
        ...
        def extraction_phase() -> Dict[str, Any]:
            """
            ...
            for page_no in range(1, max_pages + 1):
                ...
                while repairs_used <= max_repairs:
                    ...
                    if hover_context_links:
                        ...
                        links = self.collect_visible_links()
                        source_rows = self.build_source_rows(self.task, links)
...
- JSON only, no markdown."""
                        raw_map = self.ask_ai(remap_prompt)
                        parsed_map = self.safe_json_loads(raw_map, fallback={})
                    ...
                    validation = self.validate_output(data, required_fields=required_fields)
                    if validation["ok"]:
                        return {
                            "ok": True,
                            "data": data,
                    ...
                    check_timeout()
                    observe = self.observe_failure_context()
                    repair_steps = self.propose_repair(
                        self.task, 
                        validation["reason"], 
                        observe
                    )
                ...
                check_timeout()
                page_links = self.collect_visible_links()
                pagination_prompt = f"""
...
- No markdown. JSON only."""
                pagination_raw = self.ask_ai(pagination_prompt)
                pagination_parsed = self.safe_json_loads(
                    pagination_raw, 
